...
Use `const` by default at every scope; use `let` only when a binding is reassigned.

```typescript
// Module-level constants use UPPER_SNAKE_CASE for primitives.
const MAX_RETRIES = 3;
...
Use named exports. Avoid default exports; TanStack Start route modules should follow the repository's configured route export contract.
