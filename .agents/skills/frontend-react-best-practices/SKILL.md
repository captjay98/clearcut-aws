...
Performance optimization and composition patterns for React components.

**ClearCut override:** The mock at `misc/clearcut-flow/` is the UI source of truth. Use its custom primitives, Script/Night shoot tokens, and text glyphs or CSS shapes. Keep capability-gated controls accessible and preserve both themes. Contains 33 rules across 6 categories focused on reducing re-renders, optimizing bundles, component composition, and avoiding common React pitfalls.
...
```tsx
// ClearCut uses text glyphs or CSS shapes for small visual marks.
const checkGlyph = "✓";
const closeGlyph = "×";

// Keep glyphs decorative when an adjacent label already names the action.
```
...
Version and minimize localStorage data. Storage failures must remain visible to the caller.
...
function saveConfig(config: Config): boolean {
  try {
    ...
    console.warn("Unable to persist local configuration", error);
    return false;
  }
}
```
