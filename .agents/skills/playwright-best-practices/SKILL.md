...
### ClearCut Scope Override

ClearCut is a responsive web app, not a native mobile app. Use the repository's configured Playwright suite and recorded provider responses where integration data is needed. Prioritize org/project isolation, fixed-role enforcement, import and revision flows, evidence provenance, governed approvals, both themes, keyboard/focus behavior, responsive breakpoints, and report print isolation. Do not add payment, financial, email/SMS, or unrelated product flows unless a separately approved feature requires them.
