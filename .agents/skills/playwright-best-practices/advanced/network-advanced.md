...
**ClearCut evidence rule:** A HAR that contains Parallel responses is acceptable only as a repository-approved capture with a companion fixture retaining source URL, retrieval timestamp, publisher/authority classification, stance, attributable excerpt, query/research-run identity, and provider/tool receipt. Remove secrets before committing. Do not use live fallback in deterministic evidence-truth tests because it changes provenance and cost.
