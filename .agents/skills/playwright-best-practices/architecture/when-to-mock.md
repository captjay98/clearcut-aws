...
**ClearCut exception — Parallel evidence:** Parallel Search is load-bearing, and bounded Extract/conditional Monitor have distinct truthful states; none is an arbitrary third-party mock. Integration and E2E evidence tests may replay only repository-approved captures that preserve provider capability/IDs/session, source URL, retrieval timestamp, publisher/authority classification, stance, attributable excerpt, query/research-run identity, and provider/tool receipt. Never use a fabricated `route.fulfill()` response to assert evidence truth or Monitor delivery. Separate provider-contract tests exercise each enabled real adapter under an approved, quota-bounded environment.

**Mock at the boundary, test your stack end-to-end.** Mock third-party services you don't own (email or OAuth) when their behavior is not the product assertion. Never mock your own frontend-to-backend communication. Tests should prove your code works without converting external content into invented evidence.
...
└── NO → It's a third-party service.
    ├── Does it produce ClearCut evidence? → Replay an approved provenance-complete capture; never fabricate evidence.
    ├── Is it paid or rate-limited? → Mock non-evidence behavior, or use an approved quota-bounded contract test.
    ├── Is it slow or unreliable? → Prefer recorded, contract-validated fixtures.
    └── Is it a complex multi-step flow? → Use reviewed HAR recording with secrets removed.
```
